...
public:
  ...
  RotationGate(Axis axis, double teta) : Gate(2) {
    ...
    if (axis == Z) {
      (*this)(0, 0) = std::exp(-1.0 * i * teta_comp);
      (*this)(1, 1) = std::exp(i * teta_comp);
    }
    if (axis == X) {
      (*this)(0, 0) = std::cos(teta_comp);
      (*this)(0, 1) = i * -std::sin(teta_comp);
      (*this)(1, 0) = i * -std::sin(teta_comp);
      (*this)(1, 1) = std::cos(teta_comp);
    }
    if (axis == Y) {
      (*this)(0, 0) = std::cos(teta_comp);
      (*this)(0, 1) = -std::sin(teta_comp);
      (*this)(1, 0) = std::sin(teta_comp);
      (*this)(1, 1) = std::cos(teta_comp);
    }
  }
};
