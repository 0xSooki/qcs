...
private:
  ...
  std::vector<std::pair<bool, std::shared_ptr<VisualCNOT>>>
...
public:
   ...
   */
  VisualQubit(const sf::Vector2f &pos, const sf::Font &font, int id = 0)
      : id(id) {
  ...
  void addCNOTGate(VisualQubit &controlQubit) {
    if (controlQubit.getPlaceholderPosition() != placeholder_.getPosition()) {
      ...
      std::shared_ptr gate = std::make_shared<VisualCNOT>(
          controlPosition,
